...
def process_image(image_path, query):
    try:
        ...
        # Making the API request only to LLaMA
        ...
        if llama_response.status_code == 200:
            ...
            logger.info(f"Processed response from llama API: {answer}")
            responses["llama"] = answer
        else:
            logger.error(f"Error from llama API: {llama_response.status_code} - {llama_response.text}")
            responses["llama"] = f"Error from llama API: {llama_response.status_code}"
